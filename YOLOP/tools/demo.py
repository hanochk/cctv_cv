...
def detect(cfg,opt):
    ...
    clustering_lane = True # HK@@
    collect_frames_only = False  # by running the video file

    for i, (path, img, img_det, vid_cap,shapes) in tqdm(enumerate(dataset),total = len(dataset)):
        if accumulation_ll_seg:
            img_det_cp = copy.deepcopy(img_det)
            print(path)
            if i == 0:
                ll_seg_mask_acm = np.zeros((img_det.shape[:2]))# HK@@
                da_seg_mask_acm = np.zeros((img_det.shape[:2]))# HK@@
        ...
        if accumulation_ll_seg :  #np.sort(np.unique((ll_seg_mask_acm/2).astype('int')))[::-1]
            ...
            if clustering_lane and 0:
                ll_seg_mask = ll_seg_mask.astype('uint8')
                ll_seg_mask = connect_lane(ll_seg_mask)

            ll_seg_mask_acm = ll_seg_mask_acm + ll_seg_mask
            da_seg_mask_acm = da_seg_mask_acm + da_seg_mask

            mask_acm = np.zeros((ll_seg_mask_acm.shape))
            th = np.sort(np.unique(ll_seg_mask_acm.astype('int')))[::-1][1]
            th = max(th, 1) 
            if 1:
                th = 1 # one count at least telling there is segmented lane or road mark which isn;t too robust
            ...
            img_det = show_seg_result(img_det, (da_seg_mask_acm, mask_acm), _, _, is_demo=True)
    ...
    if accumulation_ll_seg:
        # road_boundary_mask = np.argwhere(mask_acm * da_seg_mask_acm>0).tolist()
        if clustering_lane:
            ll_seg_mask_acm = ll_seg_mask_acm.astype('uint8')
            ll_seg_mask_cc = connect_lane(ll_seg_mask_acm)
        if dataset.mode == 'images':

            plt.figure(0)
            plt.imshow(ll_seg_mask_cc)
            plt.savefig(str(opt.save_dir) + '/' + Path(path).name.split('.')[0] + '_mask_acm_cc' + str(i) + '.jpg')
            plt.close()
            img_with_lanes_det = show_seg_result(img_det_cp, (ll_seg_mask_cc, ll_seg_mask_cc), _, _, is_demo=True)
            img_file = os.path.join(os.path.dirname(save_path) , 'final_acm_main_lanes.jpg')
            cv2.imwrite(img_file, img_with_lanes_det)
