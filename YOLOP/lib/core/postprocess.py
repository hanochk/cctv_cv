...
def fitlane(mask, sel_labels, labels, stats, polynom_order=2):
    H, W = mask.shape
    for label_group in sel_labels:
        ...
        if if_y(samples_x):
            ...
            func = np.polyfit(samples_y, samples_x, polynom_order) # polyfiyt x=func(y) change places/roles 
            x_limits = np.polyval(func, H-1)
            
            if 1:
                if x_limits < 0 or x_limits > W:
                    if x_limits <0:
                        draw_y = np.linspace(y-y, y+h-1, h+y)
                    else:
                        draw_y = np.linspace(y-y, y+h-1, h+y)                        
                else:
            ...
            try:
                func = np.polyfit(samples_x, samples_y, polynom_order)
            except:
                pass
...
def connect_lane(image, shadow_height=0, area_th_top_2_lanes=True):
    if len(image.shape) == 3:
        gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    else:
        gray_image = image
    ...
    if area_th_top_2_lanes:# take only the two dominant lines -1 
        area_th = np.sort([stats[t][4] for t in range(1, num_labels, 1)])[::-1][1] 
    else:
        area_th = 400
    ...
    if len(selected_label) == 0:
        ...
        mask_post = fitlane(mask, split_labels, labels, stats, polynom_order=3) #draws one or more polygonal curves.
        return mask_post
...
plt.figure(0)
            plt.imshow(labels)
            plt.savefig('/notebooks/dataset/cctv/results_connect_lane/congestion-1080p_900_mask_acm_cc_labels_14.jpg')
            plt.close()
"""
